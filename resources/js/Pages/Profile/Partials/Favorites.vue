        ...
        <div class="flex justify-center dark:text-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="w-full p-4 border border-gray-200 rounded-lg shadow sm:p-8 dark:border-gray-700">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <a 
                        :href="route('profile.favorites-show')"
                        @click="issues.length === 0 && repositories.length === 0 ? $event.preventDefault() : null"
                        :class="[
                            'text-turquoise',
                            { 'cursor-not-allowed opacity-50 pointer-events-none': issues.length === 0 && repositories.length === 0 }
                        ]"
                    >
                        View all >
                    </a>
