...
<template>
    <div class="hover:ring-2 hover:ring-green">
        <button v-if="as == 'button'" type="submit" class="block w-full px-3 py-2 transition duration-150 ease-in-out">
            <slot />
        </button>
        ...
        <Link v-else :href="href" class="flex px-5 py-4 transition duration-150 ease-in-out dark:focus:bg-green">
            <slot />
        </Link>
    </div>
</template>
