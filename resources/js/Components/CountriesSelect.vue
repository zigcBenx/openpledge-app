...
export default {
  ...
  computed: {
    selectedCountryName() {
      const code = this.default || countries[0].code;
        
      if (code) return countries.find(item => item.code === code).name;
      return null;
    }
  },
  mounted() {
...
<style scoped>
  ...
  .custom-select .items {
    @apply absolute text-xs dark:bg-charcoal-gray bg-seashell rounded-md border border-green bg-seashell overflow-hidden left-0 right-0 max-h-[14rem] overflow-scroll z-10 -top-[14.3rem];
  }
